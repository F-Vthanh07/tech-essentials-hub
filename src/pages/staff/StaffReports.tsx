...
const StaffReports = () => {
  ...
  const ordersByStatus = [
    { name: 'Chờ xử lý', value: orders.filter(o => normalizeStatus(o.status) === 'pending').length, color: '#eab308' },
    { name: 'Đã thanh toán', value: orders.filter(o => normalizeStatus(o.status) === 'confirmed').length, color: '#3b82f6' },
    { name: 'Đang giao', value: orders.filter(o => normalizeStatus(o.status) === 'shipping').length, color: '#8b5cf6' },
    { name: 'Đã giao', value: orders.filter(o => normalizeStatus(o.status) === 'delivered').length, color: '#22c55e' },
    { name: 'Đã hủy', value: orders.filter(o => normalizeStatus(o.status) === 'cancelled').length, color: '#ef4444' },
  ].filter(s => s.value > 0);
