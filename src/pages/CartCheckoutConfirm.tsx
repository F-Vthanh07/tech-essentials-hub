...
interface CartCheckoutItem extends Product {
  cartItemId?: string;
  quantity: number;
...
const CartCheckoutConfirm = () => {
  ...
  const handleConfirmOrder = async () => {
    ...
    if (cartItemIds.length === 0) {
      toast.error("Sản phẩm không hợp lệ trong giỏ hàng. Vui lòng thử lại");
      return;
    }

    setIsSubmitting(true);
    try {
      const apiOrder = await orderService.createFromCartItems({
      ...
      if (fromCart) {
        // Trigger a reload of cart count instead of clearing the whole cart locally 
        // if some items were not checked out. If all was selected, clear locally.
        loadCartFromBackend(); 
      }
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 container py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl md:text-3xl font-bold">Xác nhận đơn hàng (Giỏ hàng)</h1>
        </div>

        {items.length === 0 ? (
          <Card className="p-10 text-center">
            <ShoppingBag className="w-12 h-12 mx-auto mb-4 text-muted-foreground" />
            <h2 className="text-xl font-semibold mb-2">Không có sản phẩm để thanh toán</h2>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-4">
              ...
              {items.map((item, index) => (
                <Card key={`${item.id}-${item.variantId || "no-variant"}-${index}`}>
